...
/*
$query_stamp = "SELECT date, sum(amount) as duty FROM stampdutyfirst WHERE dataentrydate >= date('" . $startdate->format('Y-m-d') ."') and dataentrydate < date('" . $enddate->format('Y-m-d') . "') and stampdutyfirst.localauthorityid<>35 GROUP BY date ORDER BY date";
*/
...
stampdutyfirst.localauthorityid<>35 and 
stampdutyfirst.localauthorityid<>0 and 
stampdutyfirst.date < date('" . $startdate->format('Y-m-d') ."') and 
stampdutyfirst.amount>0 
GROUP BY YEAR(date),MONTH(date) 
ORDER BY date";
//echo $query_stamp1."<br/>";
...
$query_stamp2 = "SELECT date, sum(amount) as duty FROM stampdutyfirst 
WHERE dataentrydate >=date('" . $datefrom ."') and dataentrydate <= date('" . $dateto . "') and 
loidentifieddate >=date('" . $datefrom ."') and loidentifieddate <= date('" . $dateto . "') and
stampdutyfirst.localauthorityid<>35 and 
stampdutyfirst.localauthorityid<>0 and
stampdutyfirst.date >= date('" . $startdate->format('Y-m-d') ."') and stampdutyfirst.amount>0 
GROUP BY date ORDER BY date";
//echo $query_stamp2."<br/>";
...
<p><a href="home.php"><img src="homelogo.jpg" class="hide-from-printer" width="35" height="35" alt="Home" /></a><img src="excellogo.jpg" alt="Export to Excel" width="30" height="31" class="hide-from-printer" onclick="fnExcelReport();"/></p>
