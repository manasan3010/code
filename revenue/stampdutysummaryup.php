...
else if ($_SESSION['MM_UserID']<35)
{$query_stampdutyfirst = "SELECT stampdutyfirst.*,  loyers1.loyername, localauthorities.name FROM stampdutyfirst, loyers1, localauthorities WHERE stampdutyfirst.dataentrydate >= date('" . $startdate->format('Y-m-d') ."') and stampdutyfirst.dataentrydate < date('" . $enddate->format('Y-m-d') . "') and stampdutyfirst.localauthorityid=localauthorities.id and stampdutyfirst.notaryid=loyers1.id and stampdutyfirst.localauthorityid=".$_SESSION['MM_UserID'] ." order by stampdutyfirst.id" ;}
else
{ switch ($_SESSION['MM_UserID'])
	{case 50:
		$district="Jaffna";
		break;
	case 51:
	...
	case 54:
		...
		break;
	case 55:
		$district="Point Pedro";
		break;
	};
if ($_GET['AGR_Confirmation']==2)
	{$query_stampdutyfirst = "SELECT stampdutyfirst.*,  loyers1.loyername, localauthorities.name FROM stampdutyfirst, loyers1, localauthorities WHERE stampdutyfirst.dataentrydate >= date('" . $startdate->format('Y-m-d') ."') and stampdutyfirst.dataentrydate < date('" . $enddate->format('Y-m-d') . "') and stampdutyfirst.localauthorityid=localauthorities.id and stampdutyfirst.notaryid=loyers1.id and localauthorities.district_council='". $district . "' order by stampdutyfirst.id" ;}
	else
	{$query_stampdutyfirst = "SELECT stampdutyfirst.*,  loyers1.loyername, localauthorities.name FROM stampdutyfirst, loyers1, localauthorities WHERE stampdutyfirst.dataentrydate >= date('" . $startdate->format('Y-m-d') ."') and stampdutyfirst.dataentrydate < date('" . $enddate->format('Y-m-d') . "') and stampdutyfirst.localauthorityid=localauthorities.id and stampdutyfirst.notaryid=loyers1.id and localauthorities.district_council='". $district . "' and
	stampdutyfirst.updateok=".$_GET['AGR_Confirmation'] . " order by stampdutyfirst.id" ;
		}
	}
...
<a href="stampdutymonthlyexceldownload.php?datefrom=<?php echo $_GET['datefrom']; ?>&amp;dateto=<?php echo $_GET['dateto']; ?>">Download Complete Data to Excel</a>
