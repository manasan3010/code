...
<style>
	...
	.close_button.black::before,
	.close_button.black::after {
		height: 8px;
		margin-top: -4px;
	}
...
<option label="Not_Started" value="Not_Started">Not Started</option>\
	...
	$(document).ready(function() {
		...
		// TO-DO: Integrate autopoulation of Model_name and Repairer_name
		// $.get("edit_src/get_repair.php", function(data, status) {
		// 	console.log(JSON.parse(data));
		// });

		$.post("edit_src/get_tasks.php", {
		...
		$.get("edit_src/get_bugs.php", function(data, status) {
			...
			window.myData = str.sort(function(a, b) {
			...
			for (var i = 0; i < myData.length; i++) {
				...
				if (myDataNew.indexOf(myData[i]['name']) == -1) {
					myDataNew.push(myData[i]['name'])
				}
			...
			$('input#name').keydown(function(event) {
				if (event.keyCode == 17) {
					...
					if ($('ul.items.dropdown-menu').css('display') == "block") {
						console.log('qw');
						$('ul.items.dropdown-menu li:first-child').click();
					}
				}
		...
		$('select#status').change(function() {
			if ($('input[name="record"]').val()) {
				...
				$('div.close_button.black').click(deleteStatus);
			}
		});
		...
		$('#savedown').click(function() {
			...
			sendData = `&record=${$('input[name="record"]').val()}`;
			$('[name=custom_module]').each(function(i, obj) {
				if ($(obj).attr('class') == 'Estimation') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('#estimation_value').val() + "&field2[]=" + $(obj).find('.date_result').val() + "&field3[]=" + $(obj).find('#description').val() + "&field4[]="
				}
				if ($(obj).attr('class') == 'In_Progress') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('.date_result').val() + "&field2[]=" + $(obj).find('#description').val() + "&field3[]=" + "&field4[]="
				}
				if ($(obj).attr('class') == 'Completed') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('.date_result').val() + "&field2[]=" + $(obj).find('#description').val() + "&field3[]=" + "&field4[]="
				}
				if ($(obj).attr('class') == 'Discarded') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('.date_result').val() + "&field2[]=" + $(obj).find('#description').val() + "&field3[]=" + "&field4[]="
				}
				if ($(obj).attr('class') == 'Not_Started') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('.date_result').val() + "&field2[]=" + $(obj).find('#description').val() + "&field3[]=" + "&field4[]="
				}
				if ($(obj).attr('class') == 'Postponed') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('.date_result').val() + "&field2[]=" + $(obj).find('#description').val() + "&field3[]=" + "&field4[]="
				}

				if ($(obj).attr('class') == 'Waiting_for_Customer') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('#customer_notified').is(':checked') + "&field2[]=" + $(obj).find('.date_result').val() + "&field3[]=" + $(obj).find('#description').val() + "&field4[]="
				}

				if ($(obj).attr('class') == 'Third_Party_Repair') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('#part_name').val() + "&field2[]=" + $(obj).find('#repairer').val() + "&field3[]=" + $(obj).find('.date_result').val() + "&field4[]=" + $(obj).find('#description').val()
				}
				if ($(obj).attr('class') == 'Waiting_for_Parts') {
					sendData += '&name[]=' + $(obj).attr('class') + "&field1[]=" + $(obj).find('#part_name_wait').val() + "&field2[]=" + $(obj).find('#model_name_wait').val() + "&field3[]=" + $(obj).find('.date_result').val() + "&field4[]=" + $(obj).find('#description').val()
				}
			...
			xhttp.open("POST", "edit_src/edit_tasks.php", true);
			xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
			xhttp.send(escape_char(sendData));
			console.log(escape_char(sendData));
			...
			xhttp4.open("POST", "edit_src/create.php", true);
	...
	function responseText(responseResponse) {
		...
		try {
			window.responseResponse = JSON.parse(responseResponse);
		} catch (err) {
			$('#error_box').html(err);
			$("#dialog-message").dialog({
				modal: true,
		...
		for (var i = 0; i < responseResponse.length - 1; i++) {
			...
			if (selectStaus == 'Waiting_for_Customer') {
				if (responseResponse[i]['field1'] == 'false') {
					$("#customer_notified").removeAttr('checked')
				}
				nowTime = responseResponse[i]['field2'];
				$(obj).find('#description').val(responseResponse[i]['field3']);
			}
	...
	function deleteStatus() {
		$(this).parent().remove();
		if ($('[name="custom_module"]').last()[0]) {
			const status = $('[name="custom_module"]').last().attr('class')
			$('#status').val(status)
		}
	}
	...
	function findIfExist() {
		...
		for (var i = 0; i < window.myData.length; i++) {
			if (window.myData[i]["name"].toLowerCase() == $('#name').val().toLowerCase()) {
				return true
			}
		}
	}
